#!/usr/bin/env bash
# Generate Qwen3-4B-Thinking-2507 reasoning-on anchor answers and score only
# the final answer tokens with the same Qwen3-4B-Thinking-2507 reference
# policy. Incomplete samples are retried in later passes until the cache is
# complete.
...
export IF_REF_ANCHOR_SHORT_RESPONSE_LENGTH=${IF_REF_ANCHOR_SHORT_RESPONSE_LENGTH:-16384}
export IF_REF_ANCHOR_SHORT_RESPONSE_TRIES=${IF_REF_ANCHOR_SHORT_RESPONSE_TRIES:-2}
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_4b_thinking2507_precompute_teacher4b_thinking2507_reasoning_anchor_scored_by_qwen3_4b_thinking2507_finalonly_retry}
...
while true; do
    ...
    try_max_response_length="${MAX_RESPONSE_LENGTH}"
    if (( try_idx <= IF_REF_ANCHOR_SHORT_RESPONSE_TRIES )); then
        try_max_response_length="${IF_REF_ANCHOR_SHORT_RESPONSE_LENGTH}"
    fi
    echo "[IF ref anchor retry] try ${try_idx} MAX_RESPONSE_LENGTH=${try_max_response_length}"
    set +e
    MAX_RESPONSE_LENGTH="${try_max_response_length}" \
    bash "${SCRIPT_DIR}/qwen3_4b_01_00_const1_ref_anchor_reasoning.sh" \
        ...
        actor_rollout_ref.rollout.response_length="${try_max_response_length}" \
