#!/usr/bin/env bash
# Freshly generate 4B non-reasoning anchor answers y0/y1 and score them
# with the same Qwen3-4B reference policy.

set -xeuo pipefail

SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
CACHE_ROOT=${CACHE_ROOT:-/NHNHOME/NHNHOME/WORKSPACE/26msit001_T_A/IFIF/if-rlvr/.cache}
...
export IF_REF_ANCHOR_PRECOMPUTE_BATCH_SIZE=${IF_REF_ANCHOR_PRECOMPUTE_BATCH_SIZE:-50000}
...
export IF_REF_ANCHOR_CACHE_PATH=${IF_REF_ANCHOR_CACHE_PATH:-${CACHE_ROOT}/if_ref_anchor_teacher4b_nonreason_train_seed1_val512_scored_by_qwen3_4b.json}
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_4b_precompute_teacher4b_nonreason_anchor_scored_by_qwen3_4b}
