#!/usr/bin/env bash
# Qwen3-4B non-reasoning policy with 4B non-reasoning final-answer anchors,
# rescored by the Qwen3-4B reference policy using non-reasoning PPL prefixes.
...
export IF_REF_ANCHOR_CACHE_PATH=${IF_REF_ANCHOR_CACHE_PATH:-${CACHE_ROOT}/if_ref_anchor_qwen3_4b_const1_train_seed1_val512_thinkfalse.json}
...
if [[ ! -s "${IF_REF_ANCHOR_CACHE_PATH}" ]]; then
    ...
    echo "Run qwen3_4b_01_00_const1_ref_anchor_reasoning.sh with ENABLE_THINKING=false and precompute enabled first." >&2
    exit 1
fi

export EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_4b_grpo_nonthink_pyx01_t4banchor_s4b_b1024_c1}
