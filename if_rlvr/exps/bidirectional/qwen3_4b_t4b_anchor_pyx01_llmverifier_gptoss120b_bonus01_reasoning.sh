#!/usr/bin/env bash
# Qwen3-4B reasoning policy with T4B anchor reward and an external
# openai/gpt-oss-120b verifier fallback.
#
# Per-rollout reward order:
#   1. policy rollout on GPUs 4,5,6,7
#   2. constraint reward is computed; rows with constraint=0 stop here
#   3. T4B policy PPL is computed only for constraint-positive rows
#   4. rows inside [lower, upper] receive the T4B +0.1 bonus
#   5. other constraint-positive rows get one GPT-OSS fallback opportunity:
#      - lower failure + verifier pass: restore constraint and add +0.1
#      - upper/invalid + verifier pass: preserve constraint and add +0.1
#
# This guarantees that the two positive bonuses never stack to +0.2.
# Validation remains constraint-only because the T4B anchor cache contains train rows only.
...
# Keep model and dataset downloads off the small system disk.
...
export RAY_TMPDIR=${RAY_TMPDIR:-${DATA_ROOT}/ray_r${RUN_SLOT}}
mkdir -p "${RAY_TMPDIR}"
...
########################### verifier server ###########################
IF_LLM_VERIFIER_DEFAULT_MODEL="openai/gpt-oss-120b"
IF_LLM_VERIFIER_LOCAL_SNAPSHOT="${HF_HUB_CACHE}/models--openai--gpt-oss-120b/snapshots/b5c939de8f754692c1647ca79fbf85e8c1e70f8a"
if [[ -f "${IF_LLM_VERIFIER_LOCAL_SNAPSHOT}/config.json" && \
      -f "${IF_LLM_VERIFIER_LOCAL_SNAPSHOT}/model.safetensors.index.json" ]]; then
    IF_LLM_VERIFIER_DEFAULT_MODEL="${IF_LLM_VERIFIER_LOCAL_SNAPSHOT}"
fi
export IF_LLM_VERIFIER_MODEL=${IF_LLM_VERIFIER_MODEL:-${IF_LLM_VERIFIER_DEFAULT_MODEL}}
export IF_LLM_VERIFIER_GPU_SET=${IF_LLM_VERIFIER_GPU_SET:-4,5,6,7}
...
export IF_LLM_VERIFIER_PYTHON=${IF_LLM_VERIFIER_PYTHON:-${VERL_DIR}/../.miniforge3/envs/verl/bin/python}
...
export IF_LLM_VERIFIER_GPU_MEM_UTIL=${IF_LLM_VERIFIER_GPU_MEM_UTIL:-0.65}
...
# Load and attach the complete precomputed Qwen3-4B reasoning anchor cache.
# PRECOMPUTE=true is the cache-loading gate in the trainer; SKIP_MISSING=true
# prevents generation for any unavailable row.
...
# This v2 cache was produced with Qwen's standard (enable_thinking=false) x-only prefix.
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_4b_grpo_think_t4banchor_pyx01_llmverifier_gptoss120b_fallback_bonus01_threshold5_b1024_c1}
