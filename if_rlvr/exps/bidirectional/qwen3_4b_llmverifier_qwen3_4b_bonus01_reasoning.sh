#!/usr/bin/env bash
# Qwen3-4B reasoning policy with cached p(y|x) anchor shaping plus an external
# Qwen/Qwen3-4B non-reasoning LLM verifier bonus.
#
...
########################### verifier server ###########################
export IF_LLM_VERIFIER_MODEL=${IF_LLM_VERIFIER_MODEL:-Qwen/Qwen3-4B}
...
export IF_LLM_VERIFIER_THRESHOLD=${IF_LLM_VERIFIER_THRESHOLD:-8}
...
if [[ "${IF_LLM_VERIFIER_START_SERVER}" =~ ^(1|true|TRUE|yes|YES|on|ON)$ ]]; then
    ...
    if [[ "${IF_LLM_VERIFIER_REPLICA_SERVERS}" =~ ^(1|true|TRUE|yes|YES|on|ON)$ ]]; then
        for replica_idx in "${!IF_LLM_VERIFIER_GPU_ARRAY[@]}"; do
            ...
            VERIFIER_LOG_FILE="${IF_LLM_VERIFIER_LOG_DIR}/qwen3_4b_gpu${verifier_gpu}_${verifier_port}.log"
            VERIFIER_CMD=(
        ...
        VERIFIER_LOG_FILE="${IF_LLM_VERIFIER_LOG_FILE:-${IF_LLM_VERIFIER_LOG_DIR}/qwen3_4b_${IF_LLM_VERIFIER_PORT}.log}"
        VERIFIER_CMD=(
            "${IF_LLM_VERIFIER_PYTHON}" -m vllm.entrypoints.cli.main serve "${IF_LLM_VERIFIER_MODEL}"
            --served-model-name "${IF_LLM_VERIFIER_MODEL}"
            --host "${IF_LLM_VERIFIER_HOST}"
...
# Use the precomputed Qwen3-4B reasoning anchor cache; do not regenerate it.
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_4b_grpo_think_pyx01_t4banchor_llmverifier_qwen3_4b_nonthink_bonus01_threshold8_b1024_c1}
