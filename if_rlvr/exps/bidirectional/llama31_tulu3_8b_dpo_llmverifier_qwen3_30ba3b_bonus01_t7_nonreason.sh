...
#   + Qwen/Qwen3-30B-A3B LLM-verifier bonus at score threshold 7
#   NO anchor / PPL reward shaping - the judge is the only non-rule signal.
#
# Tulu 3 counterpart of qwen3_17b_llmverifier_qwen3_30ba3b_bonus01_nonreason_t7.sh,
...
#   3. every constraint-positive rollout gets ONE Qwen3-30B-A3B judge call;
#      score >= 7 adds +0.1 on top of the constraint reward
#   Single reward pass, single judge call per eligible row.
#
# ---------------------------------------------------------------------------
# GPU plan - one model per GPU, no tensor sharding anywhere
# ---------------------------------------------------------------------------
#   * 8 judge replicas, TP=1, one Qwen3-30B-A3B per GPU (61 GB of bf16 weights).
#     They start on empty GPUs and are put to sleep (level 1: weights to host
#     RAM, KV freed) before the trainer touches the GPUs.
...
#     resident together. The policy rollout engine sleeps at level 2 right
#     after generation.
#   * Without the anchor pass, the trainer holds only ~1.4 GiB when the judge
#     wakes (the anchor variant left ~14 GiB of allocator residue from its
#     ref-policy log-prob pass), so the judge gets a much larger KV cache here:
#     0.85 -> ~10 GiB per replica, ~104k tokens, ~23 concurrent judgments each.
#
#   Two environment facts this box forced, kept because they cost nothing:
#     * VLLM_ALLREDUCE_USE_SYMM_MEM=0 - the container denies the CUDA multicast
#       privileges vLLM's default all-reduce rendezvous wants. No-op at TP=1.
#     * vLLM 0.11 pins numba==0.61.2, which cannot import against this image's
#       numpy 2.5; numba 0.67.0 is installed instead. The preflight below fails
#       fast if the engine worker stack ever stops importing.
...
#         AutoModelForCausalLM.from_pretrained(repo, subfolder="global_step_92")
#
# Usage:  bash llama31_tulu3_8b_dpo_llmverifier_qwen3_30ba3b_bonus01_t7_nonreason.sh
...
export IF_RLVR_RUN_ID=${IF_RLVR_RUN_ID:-tulu3_8b_dpo_q30ba3b_t7_slot${RUN_SLOT}}
...
# actor LR 5e-7. Batch 1024 x n=8 matches the sibling comparison runs.
...
# Full train split (~94.9k rows) / 1024 = 92 steps per epoch -> ~1 ckpt/epoch.
...
# No anchor/PPL shaping in this run. The reward is the IFEval constraint score
# plus a single Qwen3-30B-A3B judge bonus, so:
#   * PY/PX coefficients are 0 and the anchor cache is never loaded, which means
#     the full train split is used rather than the cached-anchor subset.
#   * if_llm_verifier_anchor_fallback_only is NOT set, so the reward manager runs
#     in its "combined" phase: every constraint-positive rollout is judged once,
#     in a single reward pass (the anchor variant needed two).
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-llama31_tulu3_8b_dpo_grpo_nonthink_llmverifier_qwen3_30ba3b_nonthink_bonus${IF_LLM_VERIFIER_BONUS_TAG}_threshold${IF_LLM_VERIFIER_THRESHOLD}_b${TRAIN_BATCH_SIZE}_c1_t1_2k}
...
export HF_CHECKPOINT_REPO_NAME=${HF_CHECKPOINT_REPO_NAME:-tulu3-8b-dpo-grpo-q30ba3b-t${IF_LLM_VERIFIER_THRESHOLD}}
