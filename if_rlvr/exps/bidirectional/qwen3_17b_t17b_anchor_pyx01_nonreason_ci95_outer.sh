#!/usr/bin/env bash
# 95% CI outer interval cache:
# lower threshold = lower CI endpoint, upper threshold = upper CI endpoint.
...
export IF_REF_ANCHOR_CACHE_PATH=${IF_REF_ANCHOR_CACHE_PATH:-${CACHE_ROOT}/if_ref_anchor_teacher17b_nonreason_train_seed1_val512_scored_by_qwen3_17b_ci95_outer_s1to8.json}
...
if [[ ! -s "${IF_REF_ANCHOR_CACHE_PATH}" ]]; then
    echo "Missing CI95 outer anchor cache: ${IF_REF_ANCHOR_CACHE_PATH}" >&2
    echo "Run build_teacher17b_ci95_anchor_caches.py first." >&2
    exit 1
fi

export EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_17b_grpo_nonthink_pyx01_t17banchor_ci95outer_s1to8_s17b_b1024_c1}
