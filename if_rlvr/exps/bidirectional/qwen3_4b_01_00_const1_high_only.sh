...
ppl_rank_reward_mode=${IF_PPL_RANK_REWARD_MODE:-high_only}  # ##6/3 ppl## both|high_only
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_4b_if_grpo_${INFER_BACKEND}_fsdp_think_${ENABLE_THINKING}_pyx_${py_given_x_reward_coeff}_pxy_${px_given_y_reward_coeff}_clip_${clipped_rollout_mode}_bsz_${train_batch_size}_ppl_${ppl_rank_reward_mode}_const1only}
...
REWARD=(
    ...
    "+if_ppl_rank_reward_mode=${ppl_rank_reward_mode}"  # ##6/3 ppl## both|high_only
)
