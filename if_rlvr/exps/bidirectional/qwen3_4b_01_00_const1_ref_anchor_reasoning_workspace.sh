...
DATA_PROCESSOR_CPU_COUNT=${DATA_PROCESSOR_CPU_COUNT:-16}
...
########################### launch ###########################
# Auto-resume on transient teardown (Ray node-death, node reboot, etc.): re-launch
# up to IF_MAX_RETRIES times. trainer.resume_mode=auto picks up the latest checkpoint,
# so a crash costs at most `save_freq` steps instead of the whole run. Set
# IF_MAX_RETRIES=1 to disable (single attempt, original behaviour).
IF_MAX_RETRIES=${IF_MAX_RETRIES:-10}
_if_rc=1
_if_attempt=0
while (( _if_attempt < IF_MAX_RETRIES )); do
    _if_attempt=$((_if_attempt + 1))
    set +e
    ...
    set -e
    if (( _if_rc == 0 )); then
        echo "[autoresume] training finished cleanly on attempt ${_if_attempt}."
        break
    fi
    echo "[autoresume] run exited rc=${_if_rc} (attempt ${_if_attempt}/${IF_MAX_RETRIES}); resume_mode=auto will resume from the last checkpoint. Retrying in 30s..." >&2
    sleep 30
done
if (( _if_rc != 0 )); then
    echo "[autoresume] exhausted ${IF_MAX_RETRIES} attempt(s); last rc=${_if_rc}." >&2
    exit "${_if_rc}"
fi
