#!/usr/bin/env bash
# GRPO for allenai/Olmo-3-7B-Instruct-DPO using cached non-thinking OLMo3
# teacher anchors. PY_GIVEN_X_REWARD_COEFF is configurable (default: 0.1).
...
export IF_RLVR_RUN_ID=${IF_RLVR_RUN_ID:-olmo3_7b_anchor_grpo_slot${RUN_SLOT}}
...
if [[ ! -s "${IF_REF_ANCHOR_CACHE_PATH}" ]]; then
    echo "Missing OLMo3 anchor cache: ${IF_REF_ANCHOR_CACHE_PATH}" >&2
    echo "Run precompute_teacher_olmo3_7b_instruct_dpo_anchor_scored_by_olmo3_7b.sh first." >&2
    exit 1
fi

PY_GIVEN_X_REWARD_TAG=${PY_GIVEN_X_REWARD_COEFF/./}
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-olmo3_7b_grpo_nonthink_anchor_pyx${PY_GIVEN_X_REWARD_TAG}_b1024_c1}
