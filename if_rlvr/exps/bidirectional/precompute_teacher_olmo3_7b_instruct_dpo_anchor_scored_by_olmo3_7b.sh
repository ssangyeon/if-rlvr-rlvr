#!/usr/bin/env bash
# Generate non-thinking OLMo3-7B Instruct-DPO anchors y0/y1 and score them
# with the same OLMo3-7B reference policy, then persist the anchor cache.
...
export IF_RLVR_RUN_ID=${IF_RLVR_RUN_ID:-olmo3_7b_anchor_precompute_slot${RUN_SLOT}}
...
export IF_REF_ANCHOR_PRECOMPUTE_BATCH_SIZE=${IF_REF_ANCHOR_PRECOMPUTE_BATCH_SIZE:-8192}
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-olmo3_7b_precompute_teacher7b_nonreason_anchor_scored_by_olmo3_7b}

# Precompute is restartable from its cache; retrying a deterministic setup/OOM
# failure ten times only hides the root error.
