#!/usr/bin/env bash
# GRPO for meta-llama/Llama-3.1-8B-Instruct using cached non-reasoning Llama
# teacher anchors with p(y|x) anchor reward coefficient 0.1.
...
export IF_RLVR_RUN_ID=${IF_RLVR_RUN_ID:-llama31_8b_anchor_grpo_slot${RUN_SLOT}}
...
if [[ ! -s "${IF_REF_ANCHOR_CACHE_PATH}" ]]; then
    echo "Missing Llama 3.1 anchor cache: ${IF_REF_ANCHOR_CACHE_PATH}" >&2
    echo "Run precompute_teacher_llama31_8b_instruct_anchor_scored_by_llama31_8b.sh first." >&2
    exit 1
fi

export EXPERIMENT_NAME=${EXPERIMENT_NAME:-llama31_8b_grpo_nonthink_pyx01_t8banchor_s8b_b1024_c1}
