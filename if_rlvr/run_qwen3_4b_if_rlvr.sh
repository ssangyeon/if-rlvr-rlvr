#!/usr/bin/env bash
# GRPO | Qwen3-4B | FSDP | Instruction-Following RLVR (IFEval)
#
# verl's examples/grpo_trainer/run_qwen3_8b_fsdp.sh with the SAME training configuration,
# but with three changes only:
#   (1) MODEL  -> Qwen/Qwen3-4B instead of Qwen/Qwen3-8B.
#   (2) DATA   -> the open-instruct IF dataset allenai/IF_multi_constraints_upto5, loaded straight
...
#   (3) REWARD -> the instruction-following verifier migrated from open-instruct, plugged in via
...
#
# Only model-appropriate default that differs from the 8B script: rollout tensor-parallel size is 1
# (Qwen3-4B fits comfortably on a single GPU; the 8B used 2). Everything else is identical and
# overridable via env vars. The reward code (if_rlvr/) is self-contained and has NO verl imports in
# its core; it does not modify any existing verl file. Training stays 100% verl-native.
#
# Prereqs:
#   conda activate verl
...
#
# Run:
#   ENABLE_THINKING=true  bash if_rlvr/run_qwen3_4b_if_rlvr.sh
#   ENABLE_THINKING=false bash if_rlvr/run_qwen3_4b_if_rlvr.sh
...
case "${DEVICE}" in
    gpu)
        ...
        rollout_tp=${rollout_tp:-1}                            # Qwen3-4B fits on one GPU (8B used 2)
